...
const VIDEO_SERVICES = {
    ...
    googledrive: {
        ...
        transform: (match) => {
            ...
            // API Key zaten bu dosyanın (1-config.js) tepesinde tanımlı
            const apiKey = API_KEYS.GOOGLE_DRIVE_API_KEY;
            
            if (!apiKey) {
                console.error('❌ Google Drive API Key bulunamadı!');
                // API key yoksa mecburen preview döner (ama VR'da çalışmaz)
                return `https://drive.google.com/file/d/${fileId}/preview`; 
            }

            // HTML sayfası değil, direkt medya akışı (stream) veren API linki:
...
function joinRoom(roomId, password = null) {
    roomsRef.child(roomId).once('value').then((snapshot) => {
        ...
        auth.signInAnonymously().then(() => {
            ...
            // İzleyici sayısını artır (Burası doğru, çünkü anlık yapılıyor)
            roomRef.child('viewers').transaction((current) => {
                return (current || 0) + 1;
            });
            
            // Presence sistemi (BURASI DÜZELTİLDİ)
            ...
            viewerPresenceRef.on('value', (snap) => {
                if (snap.val() === true) {
                    ...
                    // 1. Kural: onDisconnect ile transaction OLMAZ. Sadece remove olur.
                    ...
                    // 2. Kural: onDisconnect ile sayaç düşürülemez (güvenilmezdir).
                    // Bunun yerine 'activeViewers' listesinden silinmesi yeterlidir.
                    // Sayaç düşürme satırını tamamen sildim çünkü hataya sebep oluyordu.
                    
                    // Oda sahipliği transferi (DÜZELTİLDİ)
                    if (isRoomOwner) {
                        // Kullanıcı koparsa 'owner' bilgisini sil (null yap).
                        // Kalan kullanıcılar owner'ın null olduğunu görüp sahipliği alacak.
                        roomRef.child('owner').onDisconnect().remove();
                    }
                }
            });
